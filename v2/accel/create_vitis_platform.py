...
platform = client.create_platform_component(
    name=PFM_NAME, hw_design=hw_xsa_path, desc='Ultra96 v2 acceleration platform',
...
assert status
status = domain.set_bif(os.path.join(top_dir, 'src', 'linux.bif'))
# status = domain.generate_bif()
assert status
status = domain.set_boot_dir(petalinux_dir)
assert status
